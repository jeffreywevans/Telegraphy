...
| Data-dir override validator is probably too strict    | Valid paths with spaces can fail unnecessarily                                  | `_SAFE_PATH_PATTERN` only allows a narrow ASCII set. [data_io.py 24-37](https://github.com/jeffreywevans/Telegraphy/blob/468ee6de2d9aad8978529c5d8cfc9c204b13cd81/telegraphy/story_brief/data_io.py#L24-L37)                                                                                                                                                                                                   | Remove character allowlist; rely on path normalization and traversal checks |
...
| No published releases                                 | Harder dependency pinning and change communication                              | Repo page shows no releases. citeturn1view0                                                                                                                                                                                                                                                                                                                                                                 | Start lightweight releases once docs and artifact checks are in place       |
...
There is **not** a missing software license problem. The repo includes an MIT license, and `pyproject.toml` points to the license file. A security policy also exists. On the pure software side, that is acceptable. [LICENSE 1-21](https://github.com/jeffreywevans/Telegraphy/blob/468ee6de2d9aad8978529c5d8cfc9c204b13cd81/LICENSE#L1-L21) fileciteturn11file0L1-L1
